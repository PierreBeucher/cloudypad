...
describe('Dummy instance lifecycle', () => {
    ...
    const DUMMY_INSTANCE_NAME = "dummy-instance-lifecycle-no-snapshot-test"
    ...
    it('should deploy instance without base image snapshot', async () => {
        const providerClient = new DummyProviderClient({ config: coreConfig })
        ...
        assert.strictEqual(state.provision.output?.baseImageId, undefined, "Base image ID should not be set after deploy (snapshot disabled)")
        ...
        assert.strictEqual(infra?.baseImageId, undefined, "Base image ID should not exist in infra (snapshot disabled)")
    })

    it('should stop instance without deleting server or creating snapshot', async () => {
        const providerClient = new DummyProviderClient({ config: coreConfig })
        const manager = await providerClient.getInstanceManager(DUMMY_INSTANCE_NAME)
        const infraManager = await getDummyInfraManager()

        const stateBeforeStop = await getCurrentTestState()
        const serverIdBeforeStop = stateBeforeStop.provision.output?.instanceId
        const dataDiskIdBeforeStop = stateBeforeStop.provision.output?.dataDiskId
        assert.ok(serverIdBeforeStop, "Server ID should exist before stop")
        assert.ok(dataDiskIdBeforeStop, "Data disk ID should exist before stop")

        await manager.stop({ wait: true })
        const detailsAfterStop = await manager.getInstanceStatus()
        assert.equal(detailsAfterStop.serverStatus, ServerRunningStatus.Stopped, 'Instance should be in stopped state after stop (server not deleted)')
        ...
        assert.strictEqual(stateAfterStop.provision.output?.instanceId, serverIdBeforeStop, "Instance ID should still exist after stop (server not deleted)")
        assert.strictEqual(stateAfterStop.provision.output?.dataDiskId, dataDiskIdBeforeStop, "Data disk ID should still exist after stop")
        assert.strictEqual(stateAfterStop.provision.output?.dataDiskSnapshotId, undefined, "Data disk snapshot ID should not exist (snapshot disabled)")

        const infraAfterStop = await infraManager.getInstanceInfra()
        assert.strictEqual(infraAfterStop?.serverId, serverIdBeforeStop, "Server ID should still exist in infra after stop")
        assert.strictEqual(infraAfterStop?.dataDiskId, dataDiskIdBeforeStop, "Data disk ID should still exist in infra after stop")
        assert.strictEqual(infraAfterStop?.dataDiskSnapshotId, undefined, "Data disk snapshot ID should not exist in infra (snapshot disabled)")
    })

    it('should start instance without re-provisioning', async () => {
        const providerClient = new DummyProviderClient({ config: coreConfig })
        const manager = await providerClient.getInstanceManager(DUMMY_INSTANCE_NAME)
        const infraManager = await getDummyInfraManager()

        const stateBeforeStart = await getCurrentTestState()
        const serverIdBeforeStart = stateBeforeStart.provision.output?.instanceId
        const dataDiskIdBeforeStart = stateBeforeStart.provision.output?.dataDiskId
        assert.ok(serverIdBeforeStart, "Server ID should exist before start")
        assert.ok(dataDiskIdBeforeStart, "Data disk ID should exist before start")
        ...
        assert.strictEqual(stateAfterStart.provision.output?.instanceId, serverIdBeforeStart, "Instance ID should remain the same after start (no re-provisioning)")
        assert.strictEqual(stateAfterStart.provision.output?.dataDiskId, dataDiskIdBeforeStart, "Data disk ID should remain the same after start (no snapshot)")
        assert.strictEqual(stateAfterStart.provision.output?.dataDiskSnapshotId, undefined, "Data disk snapshot ID should not exist (snapshot disabled)")
        assert.strictEqual(stateAfterStart.provision.output?.baseImageId, undefined, "Base image ID should not exist (snapshot disabled)")

        const infraAfterStart = await infraManager.getInstanceInfra()
        assert.strictEqual(infraAfterStart?.serverId, serverIdBeforeStart, "Server ID should remain the same in infra after start")
        assert.strictEqual(infraAfterStart?.dataDiskId, dataDiskIdBeforeStart, "Data disk ID should remain the same in infra after start")
        assert.strictEqual(infraAfterStart?.baseImageId, undefined, "Base image ID should not exist in infra (snapshot disabled)")
        assert.strictEqual(infraAfterStart?.dataDiskSnapshotId, undefined, "Data disk snapshot ID should not exist in infra (snapshot disabled)")
    })
